...
export class FFTProcessor {
     ...
     */
    processBuffer(input) {
        const ret = wasm.fftprocessor_processBuffer(this.__wbg_ptr, input);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
     ...
     */
    getSize() {
        const ret = wasm.fftprocessor_getSize(this.__wbg_ptr);
        return ret >>> 0;
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('fft_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
