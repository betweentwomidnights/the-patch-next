...
const Boxes = ({ spectrumData }: { spectrumData: SpectrumDataWithTime }) => {
  ...
  const noiseSpeedRef = useRef(0);
  const targetNoiseSpeedRef = useRef(0);
  
  // Kick detection refs
  const lastBassRef = useRef(0);
  const kickImpulseRef = useRef(0);
  const kickRotationRef = useRef(new THREE.Vector3(0, 0, 0));
  ...
  const SYNC_INTERVAL = 0.1; // Sync every 100ms for tighter sync
  ...
  useFrame((state) => {
    ...
    targetNoiseSpeedRef.current = baseAudioEnergy * 0.4;
    const speedLerpFactor = isActiveRef.current ? 0.1 : 0.04;
    ...
    if (!isActiveRef.current) {
      ...
      for (let i = 0; i < BOX_COUNT_PER_SIDE ** 3; i++) {
        ...
        rotationArrays.current[idx] *= 0.9;
        rotationArrays.current[idx + 1] *= 0.9;
        rotationArrays.current[idx + 2] *= 0.9;
      }
    ...
    // Update intensity
    const currentEnergy = baseAudioEnergy * 2;
    const targetIntensity = currentEnergy + kickImpulseRef.current;
    const intensityDelta = targetIntensity - intensityRef.current;
    const rate = intensityDelta > 0 ? 0.02 : 0.03;
    intensityRef.current += intensityDelta * rate;
    ...
    for (let x = 0; x < BOX_COUNT_PER_SIDE; x++) {
      for (let y = 0; y < BOX_COUNT_PER_SIDE; y++) {
        for (let z = 0; z < BOX_COUNT_PER_SIDE; z++) {
          ...
          const swirlX = Math.sin(noiseTimeRef.current + kickSwirl) * distance * 0.1;
          const swirlY = Math.cos(noiseTimeRef.current + kickSwirl) * distance * 0.1;
          
          // Position calculation
          dummy.position.set(
            baseX + noiseFactor * (1 + (bassBand / 255) * 0.5) + swirlX * kickImpulseRef.current,
            baseY + noiseFactor * (1 + (lowMidBand / 255) * 0.5) + swirlY * kickImpulseRef.current,
            baseZ + noiseFactor * (1 + (highBand / 255) * 0.5)
          );
          ...
          dummy.rotation.set(
            noiseFactor * Math.PI * 0.2 * intensityRef.current + kickRotX,
            noiseFactor * Math.PI * 0.2 * intensityRef.current + noiseTimeRef.current * 0.1 + kickRotY,
            noiseFactor * Math.PI * 0.2 * intensityRef.current + kickRotZ
          );
